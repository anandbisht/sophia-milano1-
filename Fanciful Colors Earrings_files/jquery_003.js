 ...
 */
(function(jQuery) {
	jQuery.fn.activity = function(opts) {
		this.each(function() {
			var jQuerythis = jQuery(this);
			var el = jQuerythis.data('activity');
			if (el) {
				clearInterval(el.data('interval'));
				el.remove();
				jQuerythis.removeData('activity');
			}
			if (opts !== false) {
				opts = jQuery.extend({color: jQuerythis.css('color')}, jQuery.fn.activity.defaults, opts);
				
				el = render(jQuerythis, opts).css('position', 'absolute').prependTo(opts.outside ? 'body' : jQuerythis);
				var h = jQuerythis.outerHeight() - el.height();
				var w = jQuerythis.outerWidth() - el.width();
				var margin = {
					top: opts.valign == 'top' ? opts.padding : opts.valign == 'bottom' ? h - opts.padding : Math.floor(h / 2),
					left: opts.align == 'left' ? opts.padding : opts.align == 'right' ? w - opts.padding : Math.floor(w / 2)
				};
				var offset = jQuerythis.offset();
				if (opts.outside) {
				...
				jQuerythis.data('activity', el);
			}
		});
		return this;
	};
	
	jQuery.fn.activity.defaults = {
		segments: 12,
		space: 3,
		length: 7,
		width: 4,
	...
	jQuery.fn.activity.getOpacity = function(opts, i) {
	 ...
	 */
	var render = function() {
		return jQuery('<div>').addClass('busy');
	};
	...
	if (document.createElementNS && document.createElementNS( "http://www.w3.org/2000/svg", "svg").createSVGRect) {
		...
		render = function(target, d) {
			...
			for (var i = 0; i < d.segments; i++) {
				g.append(svg('line', {
					...
					opacity: jQuery.fn.activity.getOpacity(d, i)
				}));
			}
			return jQuery('<div>').append(el).width(2*r).height(2*r);
		};
		...
		var s = jQuery('<shape>').css('behavior', 'url(#default#VML)').appendTo('body');
			
		if (s.get(0).adj) {
		
			// VML support detected. Insert CSS rules for group, shape and stroke.
			var sheet = document.createStyleSheet();
			jQuery.each(['group', 'shape', 'stroke'], function() {
			...
			render = function(target, d) {
				...
				var el = jQuery('<group>', {coordsize: s + ' ' + s, coordorigin: o + ' ' + o}).css({top: o, left: o, width: s, height: s});
				for (var i = 0; i < d.segments; i++) {
					el.append(jQuery('<shape>', {path: 'm ' + innerRadius + ',0  l ' + (innerRadius + d.length) + ',0'}).css({
						width: s,
						height: s,
						rotation: (360 / d.segments * i) + 'deg'
					}).append(jQuery('<stroke>', {color: d.color, weight: d.width + 'px', endcap: 'round', opacity: jQuery.fn.activity.getOpacity(d, i)})));
				}
				return jQuery('<group>', {coordsize: s + ' ' + s}).css({width: s, height: s, overflow: 'hidden'}).append(el);
			};
	...
	jQuery.fn.activity = function(opts) {
		this.each(function() {
			...
			if (opts !== false) {
				...
				el = render(jQuerythis, opts).css('position', 'absolute').prependTo(opts.outside ? 'body' : jQuerythis);
				var margin = {
					top: Math.floor((jQuerythis.outerHeight() - el.height()) / 2),
					left: Math.floor((jQuerythis.outerWidth() - el.width()) / 2)
				};
				var offset = jQuerythis.offset();
				if (opts.outside) {
					el.css({top: offset.top + 'px', left: offset.left + 'px'});
...
(function(jQuery) {
	...
	var props = (function() {
		...
		var result = {};
		jQuery.each(['transitionDuration', 'transitionProperty', 'transform', 'transformOrigin'], function() {
	...
	function Transformation() {
		...
		this.reset = function() {
			jQuery.each(fn, function() {
				this.reset();
			});
		};
		this.format = function() {
			var s = '';
			jQuery.each(fn, function(k, v) {
				s += v.format() + ' ';
			});
			return s;
		};
	...
	jQuery.fn.transform = function(opts) {
		var result = this;
		if (jQuery.fn.transform.supported) {
			this.each(function() {
				var jQuerythis = jQuery(this);
				var t = transform(jQuerythis, opts);
				if (opts === undefined) {
					result = t.fn;
					return false;
				}
				var origin = opts && opts.origin ? opts.origin : '0 0';
				jQuerythis.css(props.transitionDuration, '0s')
	...
	jQuery.fn.transform.supported = !!props.transform;
	
	jQuery.fn.transition = function(css, opts) {
		...
		this.each(function() {
			...
			if (!jQuery.fn.transition.supported) {
				jQuerythis.css(css);
				if (opts.onFinish) {
					jQuery.proxy(opts.onFinish, jQuerythis)();
				}
				return;
			}
			
			var _duration = jQuerythis.css(props.transitionDuration);		
			
			function apply() {
				jQuerythis.css(props.transitionProperty, property).css(props.transitionDuration, opts.duration + 's');
				
				jQuerythis.css(css);
				if (opts.duration > 0) {
					jQuerythis.one('webkitTransitionEnd oTransitionEnd transitionend', afterCompletion);
				}
				else {
			...
			function afterCompletion() {
				jQuerythis.css(props.transitionDuration, _duration);
	...
	jQuery.fn.transition.supported = !!props.transitionProperty;
	
	jQuery.fn.transformTransition = function(opts) {
		opts = jQuery.extend({
			origin: '0 0',
			css: {}
		}, opts);
...
(function(jQuery) {
	...
	jQuery.fn.touchGallery = function(opts) {
		opts = jQuery.extend({}, jQuery.fn.touchGallery.defaults, opts);
		var thumbs = this;
	 ...
	 * Default options.
	 */
	jQuery.fn.touchGallery.defaults = {
		getSource: function() {
			return this.href;
		}
	};
	...
	function openGallery(thumbs, clickedThumb, opts) {
		...
		img.src = jQuery.proxy(opts.getSource, clickedThumb.get(0))();
	}
	...
	function showGallery(thumbs, index, clickedImage, getSrcCallback) {
		var viewport = fitToView(preventTouch(jQuery('<div id="galleryViewport">').css({
			position: 'fixed',
			top: 0,
			left: 0,
			overflow: 'hidden'
		...
		var stripe = jQuery('<div id="galleryStripe">').css({
			position: 'absolute',
			height: '100%',
			top: 0,
		...
		jQuery(window).bind('orientationchange.gallery', function() {
			fitToView(viewport);
			stripe.find('img').each(centerImage);
		});
		
		thumbs.each(function(i) {
			var page = jQuery('<div>').addClass('galleryPage').css({
				display: 'block',
				position: 'absolute',
				left: i * getInnerWidth() + 'px',
				overflow: 'hidden',
				height: '100%'
			}).width(getInnerWidth()).data('thumbs', thumbs).data('thumb', jQuery(this)).transform(false).appendTo(stripe);
			
			if (i == index) {
				var jQueryimg = jQuery(clickedImage).css({position: 'absolute', display: 'block'}).transform(false);
				makeInvisible(centerImage(index, clickedImage, jQueryimg)).appendTo(page);
				zoomIn(jQuery(this), jQueryimg, function() {
				...
				var img = new Image();
				var src = jQuery.proxy(getSrcCallback, this)();
				page.one('loadImage', function() {
					img.src = src;
				});
				img.onload = function() {
					var jQuerythis = jQuery(this).css({position: 'absolute', display: 'block'}).transform(false);
					centerImage(i, this, jQuerythis).appendTo(page.activity(false));
					page.trigger('loaded');
				};
			}
		});
	...
	function insertShade(target, onFinish) {
		var el = jQuery('<div id="galleryShade">').css({
	...
	function zoomOut(large, small, onFinish) {
		if (large.length === 0 || !jQuery.fn.transition.supported) {
			if (onFinish) {
				onFinish();
			}
			return;
	...
	function getPage(i) {
		return jQuery('#galleryStripe .galleryPage').eq(i);
	}
	...
	function setupEventListeners(el, pageWidth, currentIndex, max) {
		...
		jQuery(document).bind('keydown.gallery', function(event) {
			if (event.keyCode == 37) {
				el.trigger('prev');
			}
		...
		el.bind('touchstart', function() {
			jQuery(this).data('pan', {
				startX: event.targetTouches[0].screenX,
				lastX:event.targetTouches[0].screenX,
				startTime: new Date().getTime(),
				startOffset: jQuery(this).transform().translate.x,
				distance: function() {
		...
		.bind('touchmove', function() {
			var pan = jQuery(this).data('pan');
			jQuery(this).transform({translateBy: {x: -pan.delta()}});
			return false;
		})
	...
	function bounds(el) {
		...
		return jQuery.extend({width: el.width(), height: el.height()}, el.offset());
	}
